  ...
  <view class="page">
    ...
    <view class="content">
      <view class="one">
        ...
        <view class="li"
          >姓名 : {{ info.user_info_one.name || "展之找回用户" }}</view
        >
        <view class="li">手机号 : {{ info.user_info_one.phone }}</view>
        <view class="li"
          >公司职位 :
          {{ info.user_info_one.company_name || "展之找回用户" }}</view
        >
        <view class="li"
          >门票类型 :
          <text v-if="info.people_num">{{ info.people_num }}人票</text
          ><text v-else>单人票</text></view
        >
    ...
    <view class="footer" v-if="showFooter">
      ...
      <view class="submit" :style="'background:' + themeColors" @tap="finish">核销</view>
      <view class="submit" @tap="back">返回</view>
      <view  style="color: #606266;font-size: 24rpx;">
        注:继续核销是指核销当前用户并继续扫码,如无后续用户点击核销后返回
      </view>
    </view>
  </view>
</template>
...
export default {
  ...
  methods: {
    finish(status) {
      ...
      verifyTicketInfo({
        enroll_user_id: this.info.enroll_user_id,
        activity_equity_ids: this.checkList.join(", "),
      }).then((res) => {
        if (status == 1) {
          uni.hideLoading();
          uni.scanCode({
            ...
            success: function (res) {
              uni.navigateTo({
                url: "/pages_host/verification/info?card_num=" + res.result,
              });
            },
          });
        } else {
    ...
    back() {
      uni.navigateto({
        url: "/pages_host/verification/index",
        fail: () => {
