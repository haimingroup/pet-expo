<template>
      ...
      <view class="content">
        ...
        <view class="ticketInfo" style="margin-bottom: 40rpx">
          ...
          <view class="time">
            ...
            <span>{{ info.exhibit.start_time }}-{{ info.exhibit.end_time }}</span>
          </view>
          <view class="time">
            <u-icon name="map" size="26"></u-icon>
            <span>{{ info.exhibit.lab_name }}</span>
          </view>
        </view>
        <!-- 票务信息 -->
        <!-- <view
          ...
          v-if="!info.exhibit.many_tickets"
        >
          <view class="colorBox" :style="`background:` + themeColors"></view>
          <view class="titleStr">票务信息</view>
        </view> -->
        <!-- <view class="title" style="margin: 40rpx 0" v-if="info.exhibit.many_tickets">
          <view class="colorBox" :style="`background:` + themeColors"></view>
          <view class="titleStr">多人票选择</view>
        </view>
        <view class="ticketInfo" v-if="!info.exhibit.many_tickets">
          <view
            style="
              display: flex;
            ...
            v-if="showPic"
          >
            <p class="test">{{ info.exhibit.name }}门票</p>
            <p class="money">¥{{ info.ticket.price }}</p>
          </view>
  
          <view
            v-for="checkItem in checkList"
            :key="checkItem.activity_id"
            style="
              display: flex;
              justify-content: space-between;
              margin-bottom: 20rpx;
          ...
          >
            ...
            <p class="money">¥{{ checkItem.price || 0 }}</p>
          </view>
        </view> -->
        ...
        <!-- <view
          class="ticketInfo"
          v-if="info.exhibit.many_tickets"
          :style="current == 1 ? 'border-color:' + themeColors : ''"
          @tap="check(1, Number(info.ticket.price))"
        >
          <view style="display: flex; justify-content: space-between">
            <p class="test">单人门票</p>
            <p class="money">¥{{ Number(info.ticket.price) }}</p>
          </view>
        </view> -->
        <!-- <view
          class="ticketInfo"
          v-for="(ticket, indextic) in info.exhibit.many_tickets"
          :key="indextic"
          :style="
            current == ticket.people_num ? 'border-color:' + themeColors : ''
          "
        ...
        >
          <view
            ...
            v-if="showPic"
          >
            <p class="test">{{ ticket.people_num }}人门票</p>
            <p class="money">¥{{ Number(ticket.price) }}</p>
          </view>
        </view> -->
      </view>
      <view class="footer">
        <view class="discounts">
          <view class="left" :style="`background:` + themeColors">
            <image class="icon" src="../static/Vector.png" mode="scaleToFill" />
          </view>
          <view
            class="right"
            :style="`background:` + themeColors + `33;color:` + themeColors"
            @tap="showPop = true"
          >
          ...
          <view>
            <form>
              <from-item>
                ...
                <span
                  v-if="showRule"
                  style="color: red; font-size: 28rpx; margin-top: 10rpx"
                  >无效优惠码</span
                >
              </from-item>
            </form>
        ...
        <view class="down">
          <view class="left">
            <view style="margin-bottom: 10rpx"
              ><text>实付款：</text>
              <text style="color: #ff0000">¥{{ totalPrices }}</text></view
            >
            <view style="font-size: 28rpx; color: #7f7f7f"
              ><text>优惠-</text> <text>¥{{ dmoney }}</text></view
            >
          </view>
          <view class="right">
            <view
  ...
  import { getInfo,getTicketListInfo } from "@/api/list.js";
  ...
  export default {
    ...
    async onLoad(options) {
      ...
      if (options.check) {
        await getExhibitTicketInfo({
          exhibit_id: uni.getStorageSync("exhibit_id"),
          activity_ids: options.check,
        }).then((res) => {
      ...
      if(options.id){
        ...
        ticket_id = uni.getStorageSync("exhibit_ticket_id")
      }
      await getTicketListInfo({
        exhibit_ticket_id: ticket_id,
      }).then((res) => {
        this.info = res.data;
        this.info.exhibit.many_tickets = JSON.parse(res.data.exhibit.many_tickets);
        if (this.showPic == false) {
          this.totalPrices = Number(this.actprice);
        } else {
          this.totalPrices = Number(this.actprice) + Number(this.info.ticket.price);
        }
        uni.hideLoading();
      });
    },
    methods: {
      ...
      confirm() {
        ...
        //     console.log(
        //       this.current - 2,
        //       this.info.many_tickets,
        //       this.info.many_tickets[this.current - 2]
        //     );
        //     this.totalPrices = Number(
        //       this.info.many_tickets[this.current - 2].price
        //     );
        //   }
        // }
        verifyDisCode({
          ...
          if (res.code == 1) {
            ...
            console.log(Number(this.totalPrices) , Number(this.dmoney))
            if (Number(this.totalPrices) - Number(this.dmoney) <= 0) {
              this.totalPrices = 0;
            } else {
              this.totalPrices = Number(this.totalPrices) - Number(this.dmoney)
            }
            ...
            uni.getUserInfo({
              ...
              success: (infoRes) => {
                ...
                getOpenid({
                  ...
                  let obj = {
                    exhibit_id: that.info.exhibit.exhibit_id,
                    trade_type: 1,
